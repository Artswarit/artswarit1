...
export function MilestoneCard({
  ...
  return <Card className={`transition-all ${isPaid ? 'border-emerald-500/50 bg-emerald-500/5' : ''} ${isDisputed ? 'border-red-500/50 bg-red-500/5' : ''}`}>
      ...
      <CardContent className="space-y-4">
        ...
        {milestone.deliverables && <div className="space-y-1">
        ...
        <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
          {milestone.due_date && <div className="flex items-center gap-1">
              <Calendar className="h-4 w-4" />
              <span>Due: {format(new Date(milestone.due_date), 'MMM d, yyyy')}</span>
            </div>}
          {milestone.revision_count > 0 && <div className="flex items-center gap-1">
              <RotateCcw className="h-4 w-4" />
              <span>Revisions: {milestone.revision_count}/{milestone.max_revisions}</span>
            </div>}
          {milestone.auto_approve_at && isSubmitted && <div className="flex items-center gap-1 text-yellow-600">
              <Calendar className="h-4 w-4" />
        ...
        <div className="flex flex-wrap gap-2 pt-2 border-t">
          {/* Artist Actions */}
          {isArtist && <>
              {canStart && <Button size="sm" onClick={onStart}>
                  <Play className="h-4 w-4 mr-1" />
                  Start Milestone
                </Button>}
              {(isInProgress || isRevisionRequested) && <Button size="sm" onClick={onSubmit}>
                  <Upload className="h-4 w-4 mr-1" />
                  Submit for Review
                </Button>}
              {isPaid && <Button size="sm" variant="outline" onClick={onSubmit}>
                  <Upload className="h-4 w-4 mr-1" />
                  Upload Final Files
                </Button>}
            </>}
          ...
          {/* Client Actions */}
          {isClient && <>
              {isSubmitted && <Button size="sm" onClick={onReview}>
                  <Eye className="h-4 w-4 mr-1" />
                  Review Submission
                </Button>}
              {isApproved && <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700">
                  <DollarSign className="h-4 w-4 mr-1" />
                  Pay {formatCurrency(milestone.amount)}
                </Button>}
              {isPaid && <Button size="sm" variant="outline">
          ...
          {!isPaid && !isDisputed && (isClient || isArtist) && milestone.status !== 'pending' && <Button size="sm" variant="destructive" onClick={onDispute}>
