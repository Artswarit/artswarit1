...
const Navbar = () => {
  ...
  return <nav className="bg-white/70 glass-effect shadow-lg border-b border-gray-100 fixed w-full top-0 z-50 backdrop-blur-md transition-all">
      ...
      {/* Minimal Mobile Menu */}
      {isMobile && isOpen && <div className="md:hidden absolute top-full left-0 right-0 bg-white/90 z-50 border-b border-gray-200 shadow transition-all animate-fade-in backdrop-blur">
          <div className="px-4 py-2 space-y-1">
            {menuItems.map(item => <Link to={item.path} key={item.name} className={"relative flex items-center gap-2 px-3 py-2 text-base font-medium rounded-lg transition-colors " + (location.pathname === item.path ? "text-purple-600 after:absolute after:left-3 after:right-3 after:bottom-0 after:h-0.5 after:bg-purple-600 after:rounded-full after:scale-x-100 after:transition-transform after:duration-300" : "text-gray-700 after:bg-gray-300 hover:text-purple-600 after:absolute after:left-3 after:right-3 after:bottom-0 after:h-0.5 after:scale-x-0 hover:after:scale-x-100 after:transition-transform after:duration-300")} onClick={closeMenu} style={{
          marginLeft: "0.6rem",
          marginRight: "0.6rem"
        }}>
                {item.icon}
            ...
            <div className="mt-2">
              ...
              {user ? <>
                  <Link to={isAdmin ? "/admin-dashboard" : user.user_metadata?.role === "artist" ? "/artist-dashboard" : "/client-dashboard"} className="block px-3 py-2 text-gray-700 hover:text-purple-600 hover:bg-gray-50 rounded-md transition-colors font-medium" onClick={closeMenu}>
                    Profile
                  </Link>
                  <Button variant="ghost" className="w-full justify-start font-medium mt-1" onClick={() => {
              signOut();
              closeMenu();
            }}>
                    Logout
                  ...
                  <Link to="/login" className="block px-3 py-2 text-gray-700 hover:text-purple-600 hover:bg-gray-50 rounded-md transition-colors font-medium" onClick={closeMenu}>
                    Login
                  </Link>
                  <Link to="/signup" className="block px-3 py-2 bg-purple-600 text-white rounded-md hover:bg-purple-700 transition-all font-medium mt-1" onClick={closeMenu}>
                    Sign Up
                  </Link>
                </>}
            </div>
